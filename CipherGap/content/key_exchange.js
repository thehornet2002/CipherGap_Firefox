...
function set_pending_exchange(storageKey, nonce, data) {
    ...
    setTimeout(() => {
        ...
        if (entry) {
            ...
            globalThis.cgApi.storage.local.remove(`exchange_status_${storageKey}`).catch(() => {});
    ...
    globalThis.cgApi.runtime.onMessage.addListener((message, _sender, sendResponse) => {
        ...
        if (message.action === "clear_key") {
            (async () => {
                ...
                await globalThis.cgApi.storage.local.remove(`exchange_status_${storageKey}`);
                await globalThis.cgApi.storage.local.remove(`peer_fp_${storageKey}`);
